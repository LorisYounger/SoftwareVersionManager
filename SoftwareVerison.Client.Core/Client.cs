...
namespace SoftwareVersion.Client
{
    ...
    public class Activation
    {
        ...
        /// <param name="software">软件名称</param>
        /// <param name="version">版本</param>
        public Activation(string svmurl, string software, int version, Func<int> getComputerHash = null)
        {
            SVMURL = svmurl;
            SoftWare = software;
            Version = version;
            if (getComputerHash == null)
                GetComputerHash = GetComputerHashUnReliable;
            else
                GetComputerHash = getComputerHash;
        }
        /// <summary>
        /// 激活这台设备 返回是否激活成功
        /// </summary>
        ...
        /// <summary>
        /// 获取计算机信息计算出来的唯一计算机码, 请自己实现这个功能
        /// </summary>
        /// <returns></returns>
        public Func<int> GetComputerHash;
        /// <summary>
        /// 获取计算机信息计算出来的唯一计算机码 不可靠
        /// </summary>
        /// <returns></returns>
        public static int GetComputerHashUnReliable()
        {
            int code;
            ...
            {
                var result = md5.ComputeHash(Encoding.ASCII.GetBytes(RuntimeInformation.OSDescription));
                osd = BitConverter.ToInt32(result);
            }
            return (code + (int)RuntimeInformation.OSArchitecture) * osd;
        }
